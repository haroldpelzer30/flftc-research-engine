...
# =========================
# CREATE MONTHLY STRUCTURE FROM H4 DATA
# =========================

h4["trade_date"] = pd.to_datetime(h4["trade_date"])

h4["year"] = h4["trade_date"].dt.year
h4["month"] = h4["trade_date"].dt.month
...
monthly["prev_month_high"] = monthly["monthly_high"].shift(1)
monthly["prev_month_low"] = monthly["monthly_low"].shift(1)
...
# =========================
# DISTANCE TO MONTHLY STRUCTURE
# =========================

df["distance_to_prev_month_high_pct"] = (
    (df["close"] - df["prev_month_high"])
    / df["prev_month_high"]
) * 100

df["distance_to_prev_month_low_pct"] = (
    (df["close"] - df["prev_month_low"])
    / df["prev_month_low"]
) * 100
...
# Bullish:
# H4 RSI oversold near previous MONTHLY low

df.loc[
    (
        df["distance_to_prev_month_low_pct"].abs()
        <= NEAR_LEVEL_PCT
    )
    &
    (
...
# Bearish:
# H4 RSI overbought near previous MONTHLY high

df.loc[
    (
        df["distance_to_prev_month_high_pct"].abs()
        <= NEAR_LEVEL_PCT
    )
    &
    (
...
print("\n===== SP500 H4 RSI + MONTHLY STRUCTURE =====\n")

print(f"H4 Table Tested: {H4_TABLE}")
print("Monthly Structure: Derived from H4 data")
print(f"RSI Period: {RSI_PERIOD}")
print(f"Near Monthly Level Threshold: {NEAR_LEVEL_PCT}%")
...
signals.to_csv(
    "sp500_h4_rsi_monthly_structure_results.csv",
    index=False
)
