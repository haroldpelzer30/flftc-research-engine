...
# =========================
# CREATE WEEKLY STRUCTURE FROM H4 DATA
# =========================

h4["trade_date"] = pd.to_datetime(h4["trade_date"])

h4["year"] = h4["trade_date"].dt.year
h4["week"] = h4["trade_date"].dt.isocalendar().week.astype(int)
...
# Bullish:
# H4 RSI oversold near previous WEEKLY low

df.loc[
    (
        df["distance_to_prev_week_low_pct"].abs()
        <= NEAR_LEVEL_PCT
    )
    &
    (
...
# Bearish:
# H4 RSI overbought near previous WEEKLY high

df.loc[
    (
        df["distance_to_prev_week_high_pct"].abs()
        <= NEAR_LEVEL_PCT
    )
    &
    (
...
signals.to_csv(
    "gold_h4_rsi_weekly_structure.csv",
    index=False
)
