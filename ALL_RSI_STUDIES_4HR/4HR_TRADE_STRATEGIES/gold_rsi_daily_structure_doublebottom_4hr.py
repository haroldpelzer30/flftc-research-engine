...
RSI_PERIOD = 14
LOOKBACK_BARS = 10
NEAR_DAILY_LEVEL_PCT = 0.10
NEAR_DOUBLE_BOTTOM_PCT = 0.15
...
df["distance_to_prev_day_low_pct"] = (
    (df["close"] - df["prev_day_low"]) / df["prev_day_low"]
) * 100

df["distance_to_prev_day_high_pct"] = (
...
# and retesting recent H4 low = double bottom behavior

df.loc[
    (df["rsi"] <= 20)
    &
    (df["distance_to_prev_day_low_pct"].abs() <= NEAR_DAILY_LEVEL_PCT)
    &
    (df["distance_to_recent_low_pct"].abs() <= NEAR_DOUBLE_BOTTOM_PCT),
    "signal"
] = "BULLISH_DAILY_STRUCTURE_DOUBLE_BOTTOM"

# Bearish:
# H4 RSI overbought
# near previous daily high
# and retesting recent H4 high = double top behavior

df.loc[
    (df["rsi"] >= 80)
    &
    (df["distance_to_prev_day_high_pct"].abs() <= NEAR_DAILY_LEVEL_PCT)
    &
    (df["distance_to_recent_high_pct"].abs() <= NEAR_DOUBLE_BOTTOM_PCT),
    "signal"
] = "BEARISH_DAILY_STRUCTURE_DOUBLE_TOP"
...
bullish = signals[
    signals["signal"] == "BULLISH_DAILY_STRUCTURE_DOUBLE_BOTTOM"
]

bearish = signals[
    signals["signal"] == "BEARISH_DAILY_STRUCTURE_DOUBLE_TOP"
]
...
print("\n===== GOLD H4 RSI + DAILY STRUCTURE + DOUBLE BOTTOM/TOP =====\n")

print(f"H4 Table Tested: {H4_TABLE}")
print(f"Daily Table Used: {D1_TABLE}")
print(f"RSI Period: {RSI_PERIOD}")
print(f"Lookback Bars: {LOOKBACK_BARS}")
print(f"Near Daily Level Threshold: {NEAR_DAILY_LEVEL_PCT}%")
print(f"Near Double Bottom/Top Threshold: {NEAR_DOUBLE_BOTTOM_PCT}%")
...
signals.to_csv(
    "gold_h4_rsi_daily_structure_doublebottom_results.csv",
    index=False
)
