...
def run_ema50_ema200_cross(table_name, con):
    ...
    bullish = signals[signals["signal"] == "BULLISH_CROSS"]
    bearish = signals[signals["signal"] == "BEARISH_CROSS"]
    ...
    signals.to_csv(
        f"{asset.lower()}_ema50_ema200_results.csv",
        index=False
    )
...
def main():
    ...
    for market in MARKETS:
        try:
            results.append(run_ema50_ema200_cross(market, con))
    ...
    summary.to_csv(
        "ema50_ema200_all_markets_summary.csv",
        index=False
    )

    print("\n===== EMA50 / EMA200 CROSS ALL MARKETS SUMMARY =====\n")
    print(summary.to_string(index=False))

    print("\nCSV Export Complete:")
    print("ema50_ema200_all_markets_summary.csv")
